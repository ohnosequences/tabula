...
object TestSetting {
  ...
  object table extends Table("tabula_test_1", CompositeKey(id, name), service.region)
...
class irishService extends FunSuite {
  ...
  ignore("complex example") {
    ...
    val putResult1 = (service please PutItem(normalUser).withValue(user1)).apply(afterCreate)
    assert(putResult1.output === PutItemSuccess)
    val afterPut1 = waitFor(table, putResult1.state)

    // val putResult2 = service please (InCompositeKeyTable(afterPut1) putItem normalUser withValue user2)
    ...
    // DELETE TABLE
    val lastState = waitFor(table, putResult1.state)
