...
case object resources {
  ...
  */
  ...
  sealed trait RNil extends ResourceList {
    ...
    def :+:[H <: AnyDynamoDBResource](h: H): (H :+: RNil) = ohnosequences.tabula.resources.:+:(h, this)
  }
  object RNil extends RNil with AnyDynamoDBResource {
    ...
    val name: String = ???
    val region: ohnosequences.tabula.resources.RNil.Region = ???
    val resourceType: ohnosequences.tabula.resources.RNil.ResourceType = ???
  }
  ...
  case class ResourceListOps[RL <: ResourceList](val rl: RL) {
    ...
    def :+:[H <: AnyDynamoDBResource](h: H) : H :+: RL = ohnosequences.tabula.resources.:+:(h, rl)
  }
